...
void add_clause_to_DB() {
  ...
  for (var = *vars; var != NONE; var = *(vars += 2), vars_signs += 2) {
   ...
   push(var, POS_NEG_FOR_BACKTRACKING_STACK);
   push(sen, POS_NEG_FOR_BACKTRACKING_STACK);
...
int main(int argc, char *argv[]) {
  ...
  fprintf(fp_time, "maxsatz2013 %s %d %lld %lld %lld %d %d %d %d\n", 
  ...
  printf("maxsatz2013 %s %d %lld %lld %lld %d %d %d %d\n", 
	 	 //~ saved_input_file, ((double)(endtime-begintime)/CLOCKS_PER_SEC), 
	 	 saved_input_file,((int)(endtime.ru_utime.tv_sec - starttime.ru_utime.tv_sec)), 
	 NB_BRANCHE, NB_BACK,
	 UB, NB_VAR, INIT_NB_CLAUSE, NB_CLAUSE-INIT_NB_CLAUSE, CMTR[0]+CMTR[1]);
